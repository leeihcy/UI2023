...
bool IPAddressPrefixCheck(const IPAddressBytes& ip_address,
                          std::span<const uint8_t> ip_prefix,
                          size_t prefix_length_in_bits) {
  // Compare all the bytes that fall entirely within the prefix.
  size_t num_entire_bytes_in_prefix = prefix_length_in_bits / 8;
  if (!std::ranges::equal(ip_address.span().first(num_entire_bytes_in_prefix),
...
bool IPAddressBytes::operator<(const IPAddressBytes& other) const {
  ...
  if (size_ == other.size_) {
    ...
    int result = std::memcmp(span().data(), other.span().data(), size_);
    return result < 0;
  }
...
void IPAddressBytes::Append(std::span<const uint8_t> data) {
  CHECK_LE(data.size(), static_cast<size_t>(16 - size_));
  size_ += data.size();
  // std::span(*this).last(data.size()).copy_from(data);
  std::ranges::copy(data, std::span(*this).last(data.size()).begin());
}
...
Dns64PrefixLength ExtractPref64FromIpv4onlyArpaAAAA(const IPAddress& address) {
  ...
  auto span = std::span(address.bytes());
...
IPAddress ConvertIPv4ToIPv4EmbeddedIPv6(const IPAddress& ipv4_address,
  ...
  switch (prefix_length) {
    case Dns64PrefixLength::k96bit:
      bytes.Append(std::span(ipv6_address.bytes()).first(12u));
      bytes.Append(ipv4_address.bytes());
      return IPAddress(bytes);
    case Dns64PrefixLength::k64bit:
      bytes.Append(std::span(ipv6_address.bytes()).first(8u));
      bytes.Append(std::span(kZeroBits).first(1u));
      bytes.Append(ipv4_address.bytes());
      bytes.Append(std::span(kZeroBits).first(3u));
      return IPAddress(bytes);
    case Dns64PrefixLength::k56bit: {
      NOTREACHED(); // TODO:
      // bytes.Append(std::span(ipv6_address.bytes()).first(7u));
      // auto [first, second] = std::span(ipv4_address.bytes()).split_at(1u);
      // bytes.Append(first);
      // bytes.Append(std::span(kZeroBits).first(1u));
      // bytes.Append(second);
      // bytes.Append(std::span(kZeroBits).first(4u));
      // return IPAddress(bytes);
    }
    case Dns64PrefixLength::k48bit: {
      NOTREACHED(); // TODO:
      // bytes.Append(std::span(ipv6_address.bytes()).first(6u));
      // auto [first, second] = std::span(ipv4_address.bytes()).split_at(2u);
      // bytes.Append(first);
      // bytes.Append(std::span(kZeroBits).first(1u));
      // bytes.Append(second);
      // bytes.Append(std::span(kZeroBits).first(5u));
      // return IPAddress(bytes);
    }
    case Dns64PrefixLength::k40bit: {
      NOTREACHED(); // TODO:
      // bytes.Append(std::span(ipv6_address.bytes()).first(5u));
      // auto [first, second] = std::span(ipv4_address.bytes()).split_at(3u);
      // bytes.Append(first);
      // bytes.Append(std::span(kZeroBits).first(1u));
      // bytes.Append(second);
      // bytes.Append(std::span(kZeroBits).first(6u));
      // return IPAddress(bytes);
    }
    case Dns64PrefixLength::k32bit:
      bytes.Append(std::span(ipv6_address.bytes()).first(4u));
      bytes.Append(ipv4_address.bytes());
      bytes.Append(std::span(kZeroBits).first(8u));
      return IPAddress(bytes);
