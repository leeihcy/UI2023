...
class NET_EXPORT IPAddressBytes {
 public:
  ...
  constexpr explicit IPAddressBytes(std::span<const uint8_t> data) {
    Assign(data);
  }
  ...
  constexpr void Assign(std::span<const uint8_t> data) {
...
class NET_EXPORT IPAddress {
 public:
  ...
  // network byte order.
  constexpr explicit IPAddress(std::span<const uint8_t> address)
      : ip_address_(address) {}
  ...
  constexpr IPAddress(uint8_t b0, uint8_t b1, uint8_t b2, uint8_t b3) {
    ip_address_.Assign(std::vector<uint8_t>{b0, b1, b2, b3});
  }
