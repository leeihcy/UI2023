...
class HeapArray {
 public:
  ...
  static HeapArray CopiedFrom(std::span<const T> that) {
  ...
  static HeapArray FromOwningPointer(T* ptr, size_t size) {
    if (!size) {
  ...
  // # Checks
  // When the `size` is zero, the `ptr` must be null.
  static HeapArray FromOwningPointer(T* ptr,
                                                         size_t size,
  ...
  iterator begin() { return as_span().begin(); }
  const_iterator begin() const { return as_span().begin(); }

  iterator end() { return as_span().end(); }
  const_iterator end() const { return as_span().end(); }
  ...
  void copy_from(std::span<const T> other) { as_span().copy_from(other); }
  ...
  void copy_prefix_from(std::span<const T> other) {
  ...
  // the HeapArray.
  std::span<T> subspan(size_t offset) {
    return as_span().subspan(offset);
  }
  std::span<const T> subspan(size_t offset) const {
    return as_span().subspan(offset);
  }
  std::span<T> subspan(size_t offset, size_t count) {
    return as_span().subspan(offset, count);
  }
  std::span<const T> subspan(size_t offset,
                              size_t count) const {
    return as_span().subspan(offset, count);
  }
  ...
  std::span<T> first(size_t count) {
    return as_span().first(count);
  }
  std::span<const T> first(size_t count) const {
    return as_span().first(count);
  }

  // Returns a span over the last `count` elements of the HeapArray. A CHECK()
  // occurs if the `count` is larger than size of the HeapArray.
  std::span<T> last(size_t count) {
    return as_span().last(count);
  }
  std::span<const T> last(size_t count) const {
    return as_span().last(count);
  }
  ...
  std::span<T> leak() && {
    ...
    return std::span(leaked, dropped.size_);
  }
