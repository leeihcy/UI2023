...
void ChooseIPv6ContractionRange(std::span<const uint8_t> address,
...
void AppendIPv4Address(std::span<const uint8_t> address, CanonOutput* output) {
  DCHECK_GE(address.size(), 4u);
  for (int i = 0; i < 4; i++) {
    std::array<char, 16> str;
    // int str_len = base::SpanPrintf(str, "%d", address[i]);
    int str_len = snprintf(str.data(), sizeof(str), "%d", address[i]);
...
void AppendIPv6Address(std::span<const uint8_t> address, CanonOutput* output) {
  ...
  for (int i = 0; i <= 14;) {
    ...
    if (i == contraction_range.begin && contraction_range.len > 0) {
      ...
      int str_len = snprintf(str.data(), sizeof(str), "%x", x);
