...
bool FilePos::FileRead( void* pBuffer )
{
	...
	if ( bSuccess )
	{
		...
		if ( nNullCount )
		{
			x_AddResult( m_strIOResult, MCD_T("nulls_removed"), nullptr, MRC_COUNT, nNullCount );
...
MCD_STR CMarkup::EscapeText( MCD_CSTR szText, int nFlags )
{
	...
	while ( cSource )
	{
		...
		if ( (pFound=MCD_PSZCHR(pFind,cSource)) != nullptr )
		{
...
int CMarkup::DecodeCharUTF8( const char*& pszUTF8, const char* pszUTF8End/*=nullptr*/ )
{
	...
	// pszUTF8End can be nullptr if pszUTF8 is null terminated
