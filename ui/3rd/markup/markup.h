...
public:
	...
	// 每次从头查找该元素
	bool FirstElem( MCD_CSTR szName=nullptr );
	// 每次从当前位置查找该元素
	bool NextElem( MCD_CSTR szName=nullptr );

	// 查找子元素时，并不会修改当前位置，只是修改当前子元素位置。
	// 要继续获取找到的子元素属性，则应该调用IntoElem或者GetChildxxx
	bool FirstChildElem( MCD_CSTR szName=nullptr );
	bool NextChildElem( MCD_CSTR szName=nullptr );  
