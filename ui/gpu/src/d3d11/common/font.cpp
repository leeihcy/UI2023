#include "font.h"
#include "src/d3d11/common_def.h"
...
Font &Font::GetInstance() { return ui::D3D11Application::GetInstance().m_font; }

Font::~Font() { destroy(); }

void Font::destroy() {
#if 0
   __m_pFont10__.Release(); 
#endif
}

void Font::Init(ID3D11Device *pDevice) {
  if (!pDevice)
    return;
#if 0
  D3DX11CreateSprite(pDevice, 512, &s_pSprite10);

  D3D11_BLEND_DESC StateDesc;
  ZeroMemory(&StateDesc, sizeof(D3D11_BLEND_DESC));
  StateDesc.AlphaToCoverageEnable = FALSE;
  StateDesc.BlendEnable[0] = TRUE;
  StateDesc.SrcBlend = D3D11_BLEND_SRC_ALPHA;
  StateDesc.DestBlend = D3D11_BLEND_INV_SRC_ALPHA;
  StateDesc.BlendOp = D3D11_BLEND_OP_ADD;
  StateDesc.SrcBlendAlpha = D3D11_BLEND_ZERO;
  StateDesc.DestBlendAlpha = D3D11_BLEND_ZERO;
  StateDesc.BlendOpAlpha = D3D11_BLEND_OP_ADD;
...
ID3DX11Font *Font::getFont() {
  ...
  D3DX11CreateFont(pDevice, m_fontInfo.Height, m_fontInfo.Width,
...
void Font::beginDraw() {
  HRESULT hr = s_pSprite10->Begin(D3DX11_SPRITE_SAVE_STATE);
	...
	if (pd3dDevice)
	{
		...
		s_pSprite10->Begin(/*D3DX11_SPRITE_SORT_TEXTURE*/D3DX11_SPRITE_SORT_TEXTURE);
...
void Font::endDraw() {
  s_pSprite10->End();
	...
	ID3D11BlendState* pOriginalBlendState10 = NULL;
	ID3D11Device* pd3dDevice = NULL;
