...
void Hard3DTransform::Release() { delete this; }

TRANSFORM_TYPE Hard3DTransform::get_type() { return TRANSFORM_HARD3D; }
...
void Hard3DTransform::identity() {
  ...
  m_matrixInverse = DirectX::XMMatrixIdentity();
}

void Hard3DTransform::set_transform_rotate_origin(TRANSFORM_ROTATE_ORIGIN eX,
                                                  float fX,
                                                  TRANSFORM_ROTATE_ORIGIN eY,
                                                  float fY, float fZ) {
  m_eOriginTypeX = eX;
  m_eOriginTypeY = eY;
  m_fAnchorPointX = fX;
...
void Hard3DTransform::maprect_layer_2_view(__in RECT *rcInLayer,
                                           __out QUAD *pqInView) {
  assert(false); // TODO:
...
void Hard3DTransform::Invalidate() { m_bInvalidate = true; }

inline float deg2rad(float d) { return d * 3.14159265358979323846f / 180.0f; }

// 创建便利函数来模拟之前的访问方式
static float GetMatrixElement(const XMMATRIX& matrix, int row, int col)
{
    return XMVectorGetByIndex(matrix.r[row], col);
}

static void SetMatrixElement(XMMATRIX& matrix, int row, int col, float value)
{
    XMVECTOR rowVector = matrix.r[row];
    
    switch (col)
    {
    case 0: rowVector = XMVectorSetX(rowVector, value); break;
    case 1: rowVector = XMVectorSetY(rowVector, value); break;
    case 2: rowVector = XMVectorSetZ(rowVector, value); break;
    case 3: rowVector = XMVectorSetW(rowVector, value); break;
    }
    
    matrix.r[row] = rowVector;
}
...
void Hard3DTransform::Update() {
  ...
  if (m_xRotate != 0 || m_yRotate != 0 || m_zRotate != 0) {
    tempMatrix = DirectX::XMMatrixRotationRollPitchYaw(deg2rad(m_xRotate), deg2rad(m_yRotate), deg2rad(m_zRotate));
    m_matrix *= tempMatrix;
  }

  // 移动
  tempMatrix = tempMatrix * DirectX::XMMatrixTranslation(m_xTranslate, m_yTranslate, m_zTranslate);
  m_matrix *= tempMatrix;

  // 缩放
  if (m_xScale != 1 || m_yScale != 1 || m_zScale != 1) {
    tempMatrix = tempMatrix * DirectX::XMMatrixScaling(m_xScale, m_yScale, m_zScale);
    m_matrix *= tempMatrix;
  }
  ...
  if (m_perspective > 0) {
    ...
    SetMatrixElement(matrix, 2, 3, 1.0f / m_perspective);

    m_matrix *= matrix;
  }

  // 求出反转值
  DirectX::XMVECTOR fDet = DirectX::XMMatrixDeterminant(m_matrix);
  m_matrixInverse = DirectX::XMMatrixInverse(&fDet, m_matrix);
}
...
void Hard3DTransform::GetWorldMatrix(DirectX::XMMATRIX *pMatrix) {
  float fx, fy, fz;
  GetRotateOrigin(&fx, &fy, &fz);

  DirectX::XMMATRIX matrixRotateCenter =
      DirectX::XMMatrixTranslation(-fx, -fy, -fz);
  DirectX::XMMATRIX matrixRotateBack = DirectX::XMMatrixTranslation(fx, fy, fz);
