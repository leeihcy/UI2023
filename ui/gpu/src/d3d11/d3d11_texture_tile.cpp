...
#include "src/d3d11/common/effects.h"


namespace ui {

D3D11TextureTile::D3D11TextureTile() {
  m_width = 0;
  m_height = 0;
  m_xIndex = 0;
  m_yIndex = 0;
...
void D3D11TextureTile::Upload(ui::Rect &rcSrc, ui::UploadGpuBitmapInfo &source) {
  if (!m_pTextureBuffer) {
    create();
  }

  D3D11_MAPPED_SUBRESOURCE mappedTexture;
  UINT nSub = D3D11CalcSubresource(0, 0, 1);

  // Create的时候没有分配显存，在Map时分配显存
  HRESULT hr =
      D3D11Application::GetInstance().m_device_context->Map(m_pTextureBuffer, nSub, D3D11_MAP_WRITE_DISCARD, 0, &mappedTexture);
  if (FAILED(hr))
    return;
  ...
  D3D11Application::GetInstance().m_device_context->Unmap(m_pTextureBuffer, nSub);
  ...
  //         D3DX11SaveTextureToFile(m_pTextureBuffer, D3DX11_IFF_DDS, szTest);
...
bool D3D11TextureTile::create() {
  if (m_pTextureBuffer || m_pShaderResourceView)
    return false;

  if (!D3D11Application::GetInstance().m_device)
    return false;
  ...
  textureDesc.Format =
      ...
      // 需要在CreateDevice时添加对应的D3D11_CREATE_DEVICE_BGRA_SUPPORT Flag
  ...
  hr = D3D11Application::GetInstance().m_device->CreateShaderResourceView(
...
void D3D11TextureTile::Compositor(long xOffset, long yOffset, long vertexStartIndex,
  ...
  for (UINT p = 0; p < techDesc.Passes; ++p) {
    pTech->GetPassByIndex(p)->Apply(0, D3D11Application::GetInstance().m_device_context);
    D3D11Application::GetInstance().m_device_context->Draw(4, vertexStartIndex);
  }
