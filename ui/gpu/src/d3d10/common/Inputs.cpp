...
bool Inputs::Init(ID3D10Device *pDevice) {
  ...
  const D3D10_INPUT_ELEMENT_DESC layout[] = {
      {"POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0,
       D3D10_INPUT_PER_VERTEX_DATA, 0}, // 3*sizeof(float) = 12
      {"COLOR", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, 12,
       D3D10_INPUT_PER_VERTEX_DATA, 0}, // 4*sizeof(float) = 16
      {"TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0, 28,
       D3D10_INPUT_PER_VERTEX_DATA, 0},
  };

  D3D10_PASS_DESC PassDesc = {0};
  ID3D10EffectPass *pPass = Effects::GetInstance().m_pTechDrawRect->GetPassByIndex(0);
...
//     if (SUCCEEDED(m_pVertexBuffer->Map(D3D10_MAP_WRITE_DISCARD, 0,
//     (LPVOID*)&pVB)))
//     {
