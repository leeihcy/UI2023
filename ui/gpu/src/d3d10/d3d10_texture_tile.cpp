...
void D3D10TextureTile::Upload(ui::Rect &dirty_of_tile, ui::Rect &dirty_of_layer, ui::GpuUploadBitmap &source) {
  if (!m_pTextureBuffer) {
    create();
  }
...
void D3D10TextureTile::Compositor(long xOffset, long yOffset, long vertexStartIndex,
  ...
  d3d10::Effects::GetInstance().m_pFxVsDestPos->SetFloatVector(pos);
  ...
  if (pContext->m_bTransformValid) {
    ...
    pTech = d3d10::Effects::GetInstance().m_pTechDrawTextureMatrix;
  } else
#endif
   {
    pTech = d3d10::Effects::GetInstance().m_pTechDrawTexture;
  }

  if (pContext->m_fAlpha != 1.0f)
    d3d10::Effects::GetInstance().m_pFxAlpha->SetFloat(pContext->m_fAlpha);
  d3d10::Effects::GetInstance().m_pFxTexture10->SetResource(m_pShaderResourceView);
  ...
  if (pContext->m_fAlpha != 1.0f) {
    d3d10::Effects::GetInstance().m_pFxAlpha->SetFloat(1.0f);
  }
}
