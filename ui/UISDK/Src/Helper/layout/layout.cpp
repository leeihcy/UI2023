...
void  DesktopLayout::Arrange(WindowBase*  pWindow)
{
    ...
    else
    {
		...
		if (hWndParent)
		{
			...
			if (GetMonitorInfo(MonitorFromRect(rcParent, MONITOR_DEFAULTTOPRIMARY), &mi))
			{
				rcLimit = mi.rcWork;
			}
			else
	...
	// 递归
	//pWindow->GetLayout()->Arrange(nullptr); // <-- SetObjectPos会触发OnSize，在Window的OnSize消息中会进行layout处理，因此这里可以不调用
}
...
namespace UI
{
    ...
    void  LoadConfigWH(LPCTSTR szText, long& wh, long& whtype)
    {
        ...
        int nLength = (int)_tcslen(szText);
