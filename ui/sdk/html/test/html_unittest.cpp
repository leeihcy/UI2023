...
html::CSSTokenizer::NextTokenConfig config = {.skip_comments = false};
...
void test_tokenizer_4_3_2_Consume_comments() {
  {
    ...
    assert(tokenizer.NextToken(config).Type() == html::CSSParserTokenType::Comment);
  }
  {
  ...
  }
  {
    ...
    // 没有匹配上注释结束时，所有内容都是Comment。
    {
      ...
      html::CSSTokenizer::NextTokenConfig config_skip_comments = {.skip_comments = true};
      assert(tokenizer.NextToken(config_skip_comments).Type() == html::CSSParserTokenType::Eof);
    }
  }
}
...
void test_tokenizer_4_3_3_Consume_a_numeric_token() {
...

  {
    ...
    for (auto& [input, output] : datas) {
      ...
      auto token = tokenizer.NextToken(config);
...
void test_tokenizer_4_3_6_Consume_a_url_token() {
 {
  ...
  }
  {
    ...
    token = tokenizer.NextToken(config);
