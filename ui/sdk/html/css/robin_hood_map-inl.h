#ifndef _HTML_CSS_ROBIN_HOOD_MAP_INL_H_
#define _HTML_CSS_ROBIN_HOOD_MAP_INL_H_
...
RobinHoodMap<Key, Value>::InsertInternal(
  ...
  while (!bucket->key.IsNull()) {
    ...
    if (static_cast<unsigned>(distance) >= kPossibleBucketsPerKey) {
      ...
      assert((buckets_[num_buckets_ + kPossibleBucketsPerKey - 1])
                 .key.IsNull());
      (buckets_[num_buckets_ + kPossibleBucketsPerKey - 1]) =
          to_insert;
      return nullptr;
    }
  }
...
RobinHoodMap<Key, Value> RobinHoodMap<Key, Value>::Grow() {
  ...
  assert(new_size + kPossibleBucketsPerKey <=
