...
public:
  ...
  bool ParseStyleSheet(StyleSheetContents* style_sheet, const char *bytes, size_t size);

protected:
  void ConsumeBlockContents(CSSParserContext& context, StyleRule::RuleType rule_type, 
    CSSNestingType,
    StyleRule* parent_rule_for_nesting,
    size_t nested_declarations_start_index, std::vector<A<StyleRuleBase>>* child_rules);
  bool ConsumeDeclaration(CSSParserContext& context, StyleRule::RuleType rule_type);
  ...
  void onConsumeRuleList(CSSParserContext& context, A<StyleRuleBase>&& rule, unsigned int offset);

  A<StyleRuleBase> ConsumeAtRule(CSSParserContext& stream, AllowedRules allowed_rules);
...
A<StyleRuleBase> CreateDeclarationsRule(
  ...
  A<StyleRuleBase> ConsumeStyleRuleContents(const std::vector<CSSSelector>& selector_vector,
                                      CSSParserContext &context,
