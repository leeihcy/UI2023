...
typedef void  (*pfnEnumAliasCallback)(const wchar_t* szText, long lValue, long, long);
...
struct UIAPI AttributeSerializerWrap
{
    ...
    // IStringAttribute*  AddString(const wchar_t* szKey, void* _this, pfnStringSetter s, pfnStringGetter g);
    IStringAttribute*  AddString(const wchar_t* szKey, slot<void(const wchar_t*)>&& s, slot<const wchar_t*()>&& g);
    // IStringAttribute*  AddString(const wchar_t* szKey, const std::function<void(const wchar_t*)>& s, const std::function<const wchar_t*()>& g);
    // IStringAttribute*  AddI18nString(const wchar_t* szKey, void* _this, pfnStringExSetter s, pfnStringGetter g);
    // IStringAttribute*  AddI18nString(const wchar_t* szKey, const std::function<void(const wchar_t*, int)>& s, const std::function<const wchar_t*()>& g);
	// IStringEnumAttribute*  AddStringEnum(const wchar_t*, void* _this, pfnStringSetter s, pfnStringGetter g);

    IBoolAttribute*  AddBool(const wchar_t* szKey, bool& bBindValue);
    // IBoolAttribute*  AddBool(const wchar_t* szKey, void* _this, pfnBoolSetter s, pfnBoolGetter g);

    ILongAttribute*  AddLong(const wchar_t* szKey, long& lBindValue);
	ILongAttribute*  AddLong(const wchar_t* szKey, int& lBindValue);
    // ILongAttribute*  AddLong(const wchar_t* szKey, void* _this, pfnLongSetter s, pfnLongGetter g);
	...
	// IRegion9Attribute*  Add9Region(const wchar_t*, void* _this, pfnRectSetter s, pfnRectGetter g);

	IRenderBaseAttribute*  AddRenderBase(const wchar_t* szPrefix, IObject* pObj, IRenderBase*& pBindValue);
	ITextRenderBaseAttribute*  AddTextRenderBase(const wchar_t* szPrefix, IObject* pObj, ITextRenderBase*& pBindValue);
