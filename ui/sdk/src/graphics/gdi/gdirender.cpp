...
void GdiRenderTarget::Release() { delete this; }

// 该HDC不需要释放
HDC GdiRenderTarget::GetBindHDC() { return m_hBindDC; }
...
{
	if (RGN_COPY == nMode/* && nullptr != hRgn*/)
	{
...
{
	if (0 == nrcCount || nullptr == prc)
	{
		return SelectClipRect(nullptr, nMode);
	}
	if (1 == nrcCount)
	{
	...
	HRGN hRgn = ::ExtCreateRegion(nullptr, sizeof(RGNDATAHEADER) + nrcCount*sizeof(RECT), pRgnData);
...
void GdiRenderTarget::SetDirtyRegion(LPRECT prc, uint nrcCount) {
...
bool GdiRenderTarget::ResizeRenderBuffer(unsigned int nWidth,
...
void GdiRenderTarget::DrawString(IRenderFont *pRenderFont,
                                 DRAWTEXTPARAM *pParam) {
  if (nullptr == pParam || nullptr == pRenderFont)
    return;

  if (nullptr == pParam->prc || RECTW(pParam->prc) <= 0 ||
      RECTH(pParam->prc) <= 0)
    return;
  ...
  if ((pParam->nFormatFlag & DT_VCENTER)) {
    if ((pParam->nFormatFlag & DT_WORDBREAK)) {
      SIZE sizeText =
          pRenderFont->MeasureString(pParam->szText, rcText.Width());
...
void GdiRenderTarget::draw_string_halo(HFONT hFont, const CRect &rcText,
                                       DRAWTEXTPARAM *pParam) {
  UIASSERT(0);
#if 0
...
void GdiRenderTarget::draw_string_endalphamask(HFONT hFont, LPRECT prcText,
                                               DRAWTEXTPARAM *pParam) {
  UIASSERT(0);
#if 0
    // 只支持单行文本
...
void GdiRenderTarget::draw_string_normal(HFONT hFont, LPRECT prcText,
                                         DRAWTEXTPARAM *pParam) {
  HDC hDC = GetHDC();
  HFONT hOldFont = (HFONT)::SelectObject(hDC, hFont);
...
void GdiRenderTarget::DrawRect(LPRECT lprc, UI::Color *pColor) {
  ...
  if (pColor->a == 255) {
    ...
    BLENDFUNCTION bf = {AC_SRC_OVER, 0, 255, AC_SRC_ALPHA};
    AlphaBlend(hDC, lprc->left, lprc->top, RECTW(lprc), RECTH(lprc), hMemDC, 0,
               0, 1, 1, bf);
...
void GdiRenderTarget::TileRect(LPRECT lprc, IRenderBitmap *pRenderBitmap) {
  ...
  HBRUSH hBrush =
      ::CreatePatternBrush(pBitmap->GetBitmap()->operator HBITMAP());
...
void GdiRenderTarget::DrawLine(int x1, int y1, int x2, int y2,
                               IRenderPen *pPen) {
  if (nullptr == pPen)
    return;
...
void GdiRenderTarget::BitBlt(int xDest, int yDest, int wDest, int hDest,
                             IRenderTarget *pSrcHDC, int xSrc, int ySrc,
                             DWORD dwRop) {
  if (nullptr == pSrcHDC)
    return;
  if (pSrcHDC->GetGraphicsRenderLibraryType() !=
      GRAPHICS_RENDER_LIBRARY_TYPE_GDI)
    return;

  GdiRenderTarget *pSrcRT = static_cast<GdiRenderTarget *>(pSrcHDC);
  ::BitBlt(GetHDC(), xDest, yDest, wDest, hDest, pSrcRT->GetHDC(), xSrc, ySrc,
           dwRop);
}
...
void GdiRenderTarget::DrawBitmap(IRenderBitmap *pBitmap, int xDest, int yDest,
                                 int wDest, int hDest, int xSrc, int ySrc) {
  if (nullptr == pBitmap)
    return;

  if (pBitmap->GetGraphicsRenderLibraryType() !=
      GRAPHICS_RENDER_LIBRARY_TYPE_GDI)
    return;
...
void GdiRenderTarget::DrawBitmap(IRenderBitmap *hBitmap, int xDest, int yDest,
                                 int nDestWidth, int nDestHeight, int xSrc,
                                 int ySrc, int nSrcWidth, int nSrcHeight) {
  if (nullptr == hBitmap)
    return;
  ...
  pImage->Draw(GetHDC(), xDest, yDest, nDestWidth, nDestHeight, xSrc, ySrc,
...
void GdiRenderTarget::DrawBitmap(IRenderBitmap *hBitmap, int xDest, int yDest,
  ...
  if (nullptr == p9Region)
    return this->DrawBitmap(hBitmap, xDest, yDest, nDestWidth, nDestHeight,
  ...
  pImage->Draw(GetHDC(), xDest, yDest, nDestWidth, nDestHeight, xSrc, ySrc,
               nSrcWidth, nSrcHeight, p9Region);
}

void GdiRenderTarget::ImageList_Draw(IRenderBitmap *hBitmap, int x, int y,
...
void GdiRenderTarget::DrawBitmapEx(HDC hDC, IRenderBitmap *pBitmap,
  ...
  if (p->GetGraphicsRenderLibraryType() != GRAPHICS_RENDER_LIBRARY_TYPE_GDI) {
    if (p->GetGraphicsRenderLibraryType() ==
        GRAPHICS_RENDER_LIBRARY_TYPE_GDIPLUS) // 需要抗锯齿的情况
    {
      UIASSERT(0);
  ...
  if (pParam->nFlag & DRAW_BITMAP_STRETCH) {
    if (pParam->pRegion ||
      ...
      pImage->Draw(hDC, pParam->xDest, pParam->yDest, pParam->wDest,
                   pParam->hDest, pParam->xSrc, pParam->ySrc, pParam->wSrc,
                   pParam->hSrc, pParam->pRegion, true, pParam->nAlpha);
    } else {
      // gdi的拉伸绘制不支持抗锯齿，换gdiplus来实现
      Gdiplus::Graphics g(hDC);
      Gdiplus::Bitmap bmp(pImage->GetWidth(), pImage->GetHeight(),
                          pImage->GetPitch(), PixelFormat32bppARGB,
                          (BYTE *)pImage->GetBits());
      ...
      g.DrawImage(&bmp, rcDst, pParam->xSrc, pParam->ySrc, pParam->wSrc,
                  pParam->hSrc, Gdiplus::UnitPixel);
    }

    if (pParam->prcRealDraw) {
      ::SetRect(pParam->prcRealDraw, pParam->xDest, pParam->yDest,
    ...
    pImage->Draw(hDC, pParam->xDest, pParam->yDest, nW, nH, pParam->xSrc,
                 pParam->ySrc, nW, nH, pParam->nAlpha);

    if (pParam->prcRealDraw) {
      ::SetRect(pParam->prcRealDraw, pParam->xDest, pParam->yDest,
                pParam->xDest + nW, pParam->yDest + nH);
    }
  } else if (pParam->nFlag & DRAW_BITMAP_STRETCH_BORDER) {
    pImage->Draw(hDC, pParam->xDest, pParam->yDest, pParam->wDest,
                 pParam->hDest, pParam->xSrc, pParam->ySrc, pParam->wSrc,
                 pParam->hSrc, pParam->pRegion, false, pParam->nAlpha);
    ...
    RECT rc = {pParam->xDest, pParam->yDest, pParam->xDest + pParam->wDest,
    ...
    pImage->Draw(hDC, x, y, pParam->wSrc, pParam->hSrc, pParam->xSrc,
                 pParam->ySrc, pParam->wSrc, pParam->hSrc, pParam->nAlpha);

    if (pParam->prcRealDraw) {
      ::SetRect(pParam->prcRealDraw, x, y, x + pParam->wSrc, y + pParam->hSrc);
    }
    ...
    if (bNeedToStretch) {
      pImage->Draw(hDC, xDisplayPos, yDisplayPos, wImage, hImage, pParam->xSrc,
                   pParam->ySrc, pParam->wSrc, pParam->hSrc, pParam->pRegion,
                   true, pParam->nAlpha);
    } else {
      pImage->Draw(hDC, xDisplayPos, yDisplayPos, wImage, hImage, pParam->xSrc,
                   pParam->ySrc, wImage, hImage, pParam->nAlpha);
    }

    if (pParam->prcRealDraw) {
      ::SetRect(pParam->prcRealDraw, xDisplayPos, yDisplayPos,
    ...
    pImage->Draw(hDC, xDest, pParam->yDest, nW, nH, xSrc, pParam->ySrc, nW, nH,
                 pParam->nAlpha);

    if (pParam->prcRealDraw) {
      ::SetRect(pParam->prcRealDraw, xDest, pParam->yDest, xDest + nW,
                pParam->yDest + nH);
    }
  } else if (pParam->nFlag & DRAW_BITMAP_BITBLT_LEFTVCENTER) {
    long wDest = pParam->wSrc;
    long hDesst = pParam->hSrc;
...
//
// [注1] 如果hDC是一个MemDC的话，则必须在绘制完之后调用ModifyWorldTransform(hMemDC, nullptr, MWT_IDENTITY);
...
#define M_PI 3.14159265358979323846
	...
	if (pImage)
	{
		...
		pImage->PlgBlt(m_hDC, ptArg, pParam->xSrc, pParam->ySrc, pParam->wSrc, pParam->hSrc, nullptr, 0, 0);
...
void GdiRenderTarget::Render2DC(HDC hDstDC, Render2TargetParam *pParam) {
  ...
  if (bAlphaBlend) {
    ...
    if (wDst == wSrc && hDst == hSrc) {
      ...
      ::StretchBlt(hDstDC, xDst, ySrc, wDst, hDst, hDC, xSrc, ySrc, wSrc, hSrc,
                   SRCCOPY);
    }
  }
    ...
    if (pParam->pTransform2d)
    {
        ModifyWorldTransform(hDstDC, nullptr, MWT_IDENTITY);
        ::SetGraphicsMode(hDstDC, nOldGraphicsMode);
    }
#endif
}

void GdiRenderTarget::Render2Target(IRenderTarget *pDst,
...
void GdiRenderTarget::_FixAlpha(HDC hDC, LPCRECT lpRect, util::FixAlphaMode e,
                                WPARAM wParam) {
  RECT rc = *lpRect;
