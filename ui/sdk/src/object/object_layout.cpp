...
void ObjectLayout::SetConfigWidth(int n) {
  if (m_pLayoutParam) {
...
void ObjectLayout::SetConfigHeight(int n) {
  if (m_pLayoutParam) {
    if (m_pLayoutParam->UUID() == ICanvasLayout::UUID())
...
ILayoutParam *ObjectLayout::GetLayoutParam() { return m_pLayoutParam; }

ILayout *ObjectLayout::GetLayout() {
  Object& self = *static_cast<Object*>(this);

  Object *obj = self.GetParentObject();
  while (obj) {
...
// 如果没有，则创建一个canvas布局
ILayoutParam *ObjectLayout::GetSafeLayoutParam() {
  Object& self = *static_cast<Object*>(this);
  if (m_pLayoutParam)
  ...
  CanvasLayout layout;
  m_pLayoutParam = layout.CreateLayoutParam(self.GetIObject());
...
void ObjectLayout::CreateLayoutParam() {
  Object& self = *static_cast<Object*>(this);
  if (m_pLayoutParam)
    return;
  ...
  m_pLayoutParam = pLayout->CreateLayoutParam(self.GetIObject());
}

void ObjectLayout::DestroyLayoutParam() { SAFE_RELEASE(m_pLayoutParam); }

void ObjectLayout::SetLayoutParam(ILayoutParam *p) {
  SAFE_RELEASE(m_pLayoutParam);
  m_pLayoutParam = p;
}
...
//	获取对象自己期望的大小
//
Size ObjectLayout::GetLayoutDesiredSize() {
  Object& self = *static_cast<Object*>(this);
  if (!m_pLayoutParam) {
    CreateLayoutParam();
...
void ObjectLayout::UpdateLayoutPos() {
  if (m_pLayoutParam) {
