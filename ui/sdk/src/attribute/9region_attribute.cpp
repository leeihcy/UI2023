...
// }
void Region9Attribute::Bind(slot<void(C9Region*)> s, slot<void(C9Region*)> g)
{
...
const wchar_t*  Region9Attribute::Get()
{
	C9Region r;
    get(&r);
	...
	if (r.IsSimpleValue())
	{
		wprintf(szText, TEXT("%d"), r.left);
	}
	else if (r.IsSimpleRect())
	{
		wprintf(szText, TEXT("%d,%d,%d,%d"), r.left, r.top, r.right, r.bottom);
	}
	else
	{
		wprintf(szText, TEXT("%d,%d,%d,%d,%d,%d,%d,%d"),
				r.topleft, r.top, r.topright, r.left, r.right, r.bottomleft, r.bottom, r.bottomright);
	}
    return szText;
}

void  Region9Attribute::Set(const wchar_t* szValue)
{
	C9Region r;
...
void  Region9Attribute::Editor(SerializeParam* pData, AttributeEditorProxy* p, EditorAttributeFlag e)
{
