...
ColorAttribute::~ColorAttribute() { SAFE_DELETE(m_pIColorAttribute); }

void ColorAttribute::SetBindValue(void *p) { m_pBindValue = (Color *)p; }
...
#ifdef EDITOR_MODE
    const char *szText = m_pBindValue->GetResId();
    if (szText)
      return szText;
#endif
