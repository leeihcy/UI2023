...
#define TIMELINE_FLAG_NEED_RESET                                               \
  0x0008 // 表示一个timeline一次循环结束，下次开始前需要重置
...
public:
  ...
  void SetAnimateMgr(Animate *p) { m_pAnimateMgr = p; }

  void SetId(int nId) { m_nId = nId; }
  int GetId() { return m_nId; }

  void SetUserData(long lParam) { m_pParam = lParam; }
