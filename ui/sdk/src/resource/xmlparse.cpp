...
bool CXmlFontParse::load_from_file( CMarkup* pXml, const String& strDataSource, FontRes *pFontInfo )
{
	...
	do 
	{
		...
		for ( ;; )
		{
			...
			{
				nFontHeight = util::wtoi(strFontHeight.c_str());
			}
			if(!strOrientation.empty())
			{
				lfOrientation = util::wtoi(strOrientation.c_str());
			}
			if(!strBold.empty())
			{
				lfWeight = (util::wtoi(strBold.c_str())==1)?FW_BOLD:FW_NORMAL;
			}
			if (!strItalic.empty())
			{
				lfItalic = util::wtoi(strItalic.c_str());
			}
			if (!strUnderline.empty())
			{
				lfUnderline = util::wtoi(strUnderline.c_str());
			}
			if (!strStrikeout.empty())
			{
				lfStrikeOut = util::wtoi(strStrikeout.c_str());
			}
