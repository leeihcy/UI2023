...
void RoundRectHandler::Set(int n) {
  if (lefttop == n && leftbottom == n && righttop == n && rightbottom == n) {
    return;
  }
...
//   this->Set(r->left, r->top, r->right, r->bottom);
// }

void RoundRectHandler::Set(int nLeftTop, int nRightTop, int nLeftBottom,
                           int nRightBottom) {
  if (lefttop == nLeftTop && leftbottom == nLeftBottom &&
      righttop == nRightTop && rightbottom == nRightBottom) {
    return;
  }
...
unsigned int RoundRectHandler::GetMinWidth() {
  return std::max((lefttop + righttop), (leftbottom + rightbottom));
}
unsigned int RoundRectHandler::GetMinHeight() {
  return std::max((lefttop + leftbottom), (righttop + rightbottom));
}
...
void RoundRectHandler::CreateMaskBitmap() {
  ...
  SkCanvas canvas(m_bitmap.m_skbitmap);
  ...
  // m_bitmap.Dump("/tmp/uisdk/mask.png");
...
//  圆形
inline bool IsCircle(int nRadiusLT, int nRadiusRT, int nRadiusLB,
                     int nRadiusRB) {
  if (nRadiusLT == nRadiusLB && nRadiusLT == nRadiusRT &&
      nRadiusRT == nRadiusRB && nRadiusLT != 0)
    return true;

  return false;
}

void  PathAddPoint(SkPath& path, int x, int y)
{
    // Gdiplus::Point p;
    // p.X = x;
    // p.Y = y;
...
bool  CreateRoundRectPath3(SkPath& path, Rect* prc, 
    int nRadiusLT, int nRadiusRT, int nRadiusRB, int nRadiusLB)
{
    ...
    // 右上
    if (rt)
        path.addArc(SkRect::MakeXYWH(prc->right - rt, prc->top, rt, rt), 270, 90 );   
    else
        PathAddPoint(path, prc->right, prc->top);

    // 右下
    if (rb)
        path.addArc(SkRect::MakeXYWH(prc->right - rb, prc->bottom - rb, rb, rb), 0, 90 );   
    else
        PathAddPoint(path, prc->right, prc->bottom);

    // 左下
    if (lb)
        path.addArc(SkRect::MakeXYWH(prc->left, prc->bottom - lb, lb, lb), 90, 90 );    
    else
        PathAddPoint(path, prc->left, prc->bottom);

    // 左上
    if (lt)
        path.addArc(SkRect::MakeXYWH(prc->left, prc->top, lt, lt), 180, 90 );            
    else
        PathAddPoint(path, prc->left, prc->top);

    path.close();
    return true;
}

void RoundRectHandler::CreatePath(SkPath& path) {
  if (IsCircle(lefttop, righttop, rightbottom, leftbottom)) {
    path.addCircle(lefttop, lefttop, lefttop);
    return;
  }
  ...
  CreateRoundRectPath3(path, &rc, lefttop, righttop, leftbottom, rightbottom);
}
...
void RoundRectHandler::RequestBuffer(unsigned int nWidth,
  ...
  if (m_bitmap.IsNull()) {
    ...
    m_bitmap.Create(nWidth, nHeight);
...
// void RoundRectHandler::HandleYLine(byte* pDstBits, int nDstPitch, int yFrom,
// int yTo)
// {
...
bool RoundRectHandler::Apply2Bitmap(HBITMAP hBitmap, bool bTopDown,
                                    RECT *prcRegion, RECT *prcClip) {
  ...
  if (lefttop) {
    ...
    if (prcClip) {
      RECT rcRealDst = {data.xDst, data.yDst, data.xDst + data.width,
